...
#-              @groups[group]=Mutex.new
              ...
              t= Thread.new do
                ...
                executor=nil
                  ...
                  my_pipe=pipes[my_group]
              #parent,pipe,store,current_var,stream_var,chain,final
                  hop_warn "GROUP THREAD #{Thread.current} start for #{my_group} (#{self})"
...
#                hop_warn "GROUP THREAD #{t} executor for #{group} #{executor}"
                ...
                hop_warn "GROUP THREAD #{Thread.current} end: #{Thread.current['result'].inspect}"
              end
              @threads[group]=t
